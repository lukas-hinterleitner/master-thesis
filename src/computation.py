...
from src.config.dataset import get_dataset_config  # dataset‑specific settings
...
def build_bm25_index(original_dataset_tokenized: Dataset | IterableDataset) -> BM25Okapi:
...
# ---------------------------------------------------------------------------
# Gradient helpers (wrappers around existing project utilities)
# ---------------------------------------------------------------------------

def __get_paraphrased_gradients(
    paraphrased_sample: dict,
    model: PreTrainedModel,
    *,
    is_model_generated: bool,
    tokenizer: PreTrainedTokenizer | None = None,
    paraphrased_config: dict | None = None,
) -> dict[str, torch.Tensor]:
    """
    Compute gradients for the provided paraphrased sample using the specified
    pre-trained model and additional parameters. Depending on the
    `is_model_generated` flag, the function handles two distinct paths: one for
    user-provided paraphrases and one for model-generated paraphrases.

    :param paraphrased_sample: The paraphrased sample to be used for gradient
        computation.
    :type paraphrased_sample: dict
    :param model: The pre-trained model from which gradients are computed.
    :type model: PreTrainedModel
    :param is_model_generated: Flag indicating whether the paraphrased sample
        is generated by the model (`True`) or provided externally (`False`).
    :type is_model_generated: bool
    :param tokenizer: The tokenizer matching the given model to process the
        paraphrased sample. Optional; defaults to `None`.
    :return: A dictionary where keys represent feature names and values are the
        computed gradients as tensors.
    :rtype: dict[str, torch.Tensor]
    """

    if not is_model_generated:
        return get_gradients(model, paraphrased_sample)

    # === Model‑generated paraphrase path (unchanged logic) ================
    ...
    return get_gradients(model, paraphrased_sample_model_generated)
...
def __calculate_flattened_similarities(
    original_dataset_tokenized: Dataset,
    paraphrased_dataset_iterable: Dataset | IterableDataset,
    model: PreTrainedModel,
    top_k: int,
    is_model_generated: bool,
    tokenizer: PreTrainedTokenizer | None = None,
    ...
    :param paraphrased_dataset_iterable: An iterable or dataset containing paraphrased data samples.
    ...
    :param is_model_generated: A boolean specifying whether the paraphrased dataset is generated
        by the model.
    :param tokenizer: (Optional) A tokenizer to preprocess text input, required if `is_model_generated`
        is True.
    ...
    paraphrased_config = (
        get_dataset_config(model, sft_messages_key="paraphrased_messages")
...
def __calculate_random_projected_similarities(
    ...
    :param paraphrased_dataset_iterable: An iterable or dataset containing the paraphrased samples.
    ...
    :param tokenizer: Tokenizer used for processing text data, optional.
...
def __calculate_layerwise_dot_products(
    ...
    :param paraphrased_dataset_iterable: An iterable of paraphrased dataset, which may be either a
    ...
    :param tokenizer: Optional pre-trained tokenizer object used in processing paraphrased samples.
...
def calculate_paraphrased_gradient_similarities(
    ...
    return __calculate_flattened_similarities(
        ...
        paraphrased_dataset_iterable=paraphrased_dataset_tokenized,
        model=model,
        top_k=top_k,
...
def calculate_model_generated_gradient_similarities(
    ...
    :param model: The pre-trained model used for calculating gradient similarities
        between datasets.
    :param tokenizer: The tokenizer corresponding to the model for processing text
        data.
    ...
    return __calculate_flattened_similarities(
        original_dataset_tokenized=original_dataset_tokenized,
        paraphrased_dataset_iterable=paraphrased_dataset,
        model=model,
        top_k=top_k,
...
def calculate_model_generated_random_projected_gradient_similarities(
    ...
    :type model: PreTrainedModel
    :param tokenizer: Pre-trained tokenizer used for dataset tokenization.
...
def calculate_model_generated_layer_dot_products(
    ...
    :param tokenizer: PreTrainedTokenizer
        The tokenizer associated with the model for tokenizing the data if needed.
    :param top_k: Int
